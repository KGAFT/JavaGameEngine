...
public class VulkanLogger {
    ...
    public int invoke(int messageSeverity, int messageType, long pCallbackData, long pUserDat) {
        ...
        if(!message.getVulkanMessage().contains("Semaphore must not be currently signaled") && !message.getVulkanMessage().contains("encountered the following validation error at vkCmdDrawIndexed")){
            if(message.isError()){
