...
public class GBuffer implements IFrameBuffer, WindowResizeCallBack{
    ...
    private void loadTextures(){
        Shader.uniformInt(textureBlocks.get(0), "positionsMap");
        Shader.uniformInt(textureBlocks.get(1), "normalMap");
        Shader.uniformInt(textureBlocks.get(2), "albedoMap");
        Shader.uniformInt(textureBlocks.get(3), "metallicMap");
        Shader.uniformInt(textureBlocks.get(4), "roughnessMap");
        Shader.uniformInt(textureBlocks.get(5), "aoMap");
        Shader.uniformInt(textureBlocks.get(6), "emissiveMap");
        GL33.glActiveTexture(GL33.GL_TEXTURE0+textureBlocks.get(0));
        GL33.glBindTexture(GL33.GL_TEXTURE_2D, positionsTexture);
        GL33.glActiveTexture(GL33.GL_TEXTURE0+textureBlocks.get(1));
        GL33.glBindTexture(GL33.GL_TEXTURE_2D, normalTexture);
        GL33.glActiveTexture(GL33.GL_TEXTURE0+textureBlocks.get(2));
        GL33.glBindTexture(GL33.GL_TEXTURE_2D, albedoTexture);
        GL33.glActiveTexture(GL33.GL_TEXTURE0+textureBlocks.get(3));
        GL33.glBindTexture(GL33.GL_TEXTURE_2D, metallicTexture);
        GL33.glActiveTexture(GL33.GL_TEXTURE0+textureBlocks.get(4));
        GL33.glBindTexture(GL33.GL_TEXTURE_2D, roughnessTexture);
        GL33.glActiveTexture(GL33.GL_TEXTURE0+textureBlocks.get(5));
        GL33.glBindTexture(GL33.GL_TEXTURE_2D, aoTexture);
        GL33.glActiveTexture(GL33.GL_TEXTURE0+textureBlocks.get(6));
    ...
    private void create(){
        ...
        GL33.glBindFramebuffer(GL33.GL_FRAMEBUFFER, 0);
        if(textureBlocks.size()==0){
            for(int i = 0; i<=7; i++){
                textureBlocks.add(Texture.textureCount+i);
