...
import com.kgaft.OpenlGLGameEngine.Engine.VertexObjects.ElementBufferObject;
import com.kgaft.OpenlGLGameEngine.Engine.VertexObjects.VertexArrayObject;
import com.kgaft.OpenlGLGameEngine.Engine.VertexObjects.VertexBufferObject;
...
public class ModelLoader {
    ...
    private Mesh processMesh(AIMesh mesh, AIScene scene, String directory){
        ...
        while(textureBuffer.remaining()>0){
            AIVector2D textureCoords = AIVector2D.create(textureBuffer.get());
            if(textureCoords.address()!=0){
                uvs.add(textureCoords.x());
                uvs.add(textureCoords.y());
            }
        }
            ...
            if(texture==null){
                switch (textureType){
                    case Assimp.aiTextureType_DIFFUSE:
                        texture = Texture.loadTextureFromFile(textureDirectory+"/"+path.dataString(), c, "diffuseTexture");
                        break;
                    case Assimp.aiTextureType_SPECULAR:
                        texture = Texture.loadTextureFromFile(textureDirectory+"/"+path.dataString(), c, "specularTexture");
                        break;
                }
            }
            if(texture!=null){
